...
const xlsxFile = path.join(resultsDir, "appium-e2e-results.xlsx");
...
`;
  ...
  results.forEach(r => {
    ...
    report += `| ${r.testId} | ${r.category} | ${r.title} | ${statusEmoji} ${r.status} | ${r.details.substring(0, 50)}... |\n`;
  });
  ...
  report += `- Total Duration: See individual test timestamps\n`;
